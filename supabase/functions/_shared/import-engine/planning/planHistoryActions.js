function getCoreFieldHistoryReason(field) {
    switch (field) {
        case 'legalFirstName':
...
function getSensitiveFieldHistoryReason(field) {
    switch (field) {
        case 'workEmail':
...
export function planHistoryActions(memberActionPlan, coreMemberFieldUpdatePlan, sensitiveDetailPlan) {
    const rows = [];
    ...
    const coreFieldUpdatedCount = rows.filter((row) => row.actionType === 'core_field_updated').length;
    const sensitiveFieldUpdatedCount = rows.filter((row) => row.actionType === 'sensitive_field_updated').length;
